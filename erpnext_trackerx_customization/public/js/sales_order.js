frappe.ui.form.on('Sales Order', {
    ...
    refresh(frm) {
        ...
        // Initialize tree view after form is loaded
        setTimeout(() => {
            setup_tree_view(frm);
        }, 500);
    },
...
function bind_tree_events(frm) {
    ...
    $('.custom-color-input').off('input').on('input', function() {
        const item_index = $(this).data('item-index');
        frm.tree_data[item_index].custom_color = $(this).val();
        frm.dirty();
    });
    
    $('.line-item-input').off('input').on('input', function() {
        const item_index = $(this).data('item-index');
    ...
    $('.ex-fty-date-input').off('change').on('change', function() {
        const item_index = $(this).data('item-index');
    ...
    // Size fields change
    $('.size-input').off('input').on('input', function() {
        const item_index = $(this).data('item-index');
    ...
    // Add size button
    $('.add-size-btn').off('click').on('click', function(e) {
        e.stopPropagation();
        const item_index = $(this).data('item-index');
    ...
    $('.remove-size-btn').off('click').on('click', function(e) {
        e.stopPropagation();
        const item_index = $(this).data('item-index');
        const size_index = $(this).data('size-index');
