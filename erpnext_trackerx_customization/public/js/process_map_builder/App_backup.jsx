...
export function App() {
  ...
  useEffect(() => {
  const loadFG = async () => {
    const details = await fetchItemDetails("FG-PAN-style_master_desc-red-red-000001");
    console.log("FG Components:", details.fg_components);
  };
  loadFG();
}, []);
  ...
  try {
    ...
    const result = await response.json();
    return result.data; // contains fg_components
  } catch (error) {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const [opData, pgData, streamData, operationData, pmData, itemData] = await Promise.all([
  ...
  console.log("operations", operation, items);
