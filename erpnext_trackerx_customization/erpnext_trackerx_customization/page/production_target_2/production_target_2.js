...
class SimpleProductionTargetManager {
    ...
    async loadConfiguration() {
        ...
        try {
            ...
            this.currentConfiguration = response.message;
    ...
    populateStylesTable() {
        ...
        if (!this.currentConfiguration || !this.selectedStyle) {
            return;
        }

        // OPTIMIZED: Use data from get_style_configuration API
        const config = this.currentConfiguration;
        const existing = config.existing_config;

        const sam = existing ? existing.sam : config.calculated_sam;
        const operator = existing ? existing.operator : config.default_operator_count;
        const efficiency = existing ? existing.efficiency : 0;
        const target = existing ? existing.hour_target : 0;
        const hasConfig = !!existing;

        const row = `
            <tr data-style="${config.style_name}" class="${hasConfig ? 'table-light' : ''}">
                <td>
                    <div class="style-info">
                        <strong>${config.style_item_name || config.style_name}</strong>
                        <br><small class="text-muted">${config.style_name}</small>
                ...
                <td>
                    <div class="input-group input-group-sm">
                        ...
                        <div class="input-group-append">
                            <button class="btn btn-outline-secondary calc-sam-btn" data-style="${config.style_name}" title="Calculate SAM">
                                <i class="fa fa-calculator"></i>
                            </button>
                        </div>
                    </div>
                ...
                <td>
                    <button class="btn btn-sm btn-info history-btn" data-style="${config.style_name}" title="View History">
    ...
    async saveAllChanges() {
        ...
        try {
            // OPTIMIZED: Save single configuration instead of bulk
            if (this.pendingChanges.size === 1 && this.selectedCell && this.selectedStyle) {
                ...
                if (response.message.status === 'success') {
                    ...
                    this.clearPendingChanges();
                    // Reload the specific style configuration
                    await this.onStyleSelected(this.selectedStyle.name);
                }
            } else {
