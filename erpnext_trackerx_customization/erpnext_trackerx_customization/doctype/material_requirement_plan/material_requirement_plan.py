...
form_grid_templates = {"items": "templates/form_grid/material_requirement_plan_grid.html"}


class MaterialRequirementPlan(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from erpnext_trackerx_customization.erpnext_trackerx_customization.doctype.material_requirement_plan_item.material_requirement_plan_item import MaterialRequirementItem

		amended_from: DF.Link | None
		company: DF.Link
		customer: DF.Link | None
		items: DF.Table[MaterialRequirementItem]
		...
		for so_no in so_items.keys():
			for item in so_items[so_no].keys():
				already_indented = frappe.db.sql(
					"""select sum(qty)
					from `tabMaterial Requirement Plan Item`
				...
				if actual_so_qty and (flt(so_items[so_no][item]) + already_indented > actual_so_qty):
					frappe.throw(
						_(
							"Material Requirement Plan of maximum {0} can be made for Item {1} against Sales Order {2}"
	...
	def validate(self):
		...
		self.validate_material_requirement_plan_type()
		...
		# Though the creation of Material Requirement Plan from a Production Plan can be rethought to fix this
	...
	def validate_material_requirement_plan_type(self):
		"""Validate fields in accordance with selected type"""

		if self.material_requirement_plan_type != "Customer Provided":
			self.customer = None
	...
	def set_title(self):
		...
		if not self.title:
			...
			self.title = _("{0} Request for {1}").format(_(self.material_requirement_plan_type), items)[:100]

	def on_submit(self):
		self.update_requested_qty_in_production_plan()
		self.update_requested_qty()
		if self.material_requirement_plan_type == "Purchase" and frappe.db.exists(
			"Budget", {"applicable_on_material_requirement_plan": 1, "docstatus": 1}
		):
	...
	def check_modified_date(self):
		mod_db = frappe.db.sql("""select modified from `tabMaterial Requirement Plan` where name = %s""", self.name)
	...
	def get_mr_items_ordered_qty(self, mr_items):
		...
		if self.material_requirement_plan_type in ("Material Issue", "Material Transfer", "Customer Provided"):
			doctype = frappe.qb.DocType("Stock Entry Detail")
			qty_field = doctype.transfer_qty
		elif self.material_requirement_plan_type == "Manufacture":
		...
		if doctype and qty_field:
			query = (
				...
				.select(doctype.material_requirement_item, Sum(qty_field))
				.where(
					(doctype.material_requirement == self.name)
					& (doctype.material_requirement_item.isin(mr_items))
					& (doctype.docstatus == 1)
				)
				.groupby(doctype.material_requirement_item)
			)
	...
	def update_completed_qty(self, mr_items=None, update_modified=True):
		if self.material_requirement_plan_type == "Purchase":
			return
				...
				if self.material_requirement_plan_type in ("Material Issue", "Material Transfer", "Customer Provided"):
					...
					if mr_qty_allowance:
						...
						if d.ordered_qty and flt(d.ordered_qty, precision) > flt(allowed_qty, precision):
							frappe.throw(
								_(
									"The total Issue / Transfer quantity {0} in Material Requirement Plan {1}  cannot be greater than allowed requested quantity {2} for Item {3}"
						...
						frappe.throw(
							_(
								"The total Issue / Transfer quantity {0} in Material Requirement Plan {1} cannot be greater than requested quantity {2} for Item {3}"
		...
		self._update_percent_field(
			{
				"target_dt": "Material Requirement Plan Item",
	...
	def update_requested_qty_in_production_plan(self):
		production_plans = []
		for d in self.get("items"):
			if d.production_plan and d.material_requirement_plan_plan_item:
				qty = d.qty if self.docstatus == 1 else 0
				frappe.db.set_value(
					"Material Requirement Plan Plan Item", d.material_requirement_plan_plan_item, "requested_qty", qty
				)
...
def update_completed_and_requested_qty(stock_entry, method):
	if stock_entry.doctype == "Stock Entry":
		material_requirement_plan_map = {}

		for d in stock_entry.get("items"):
			if d.material_requirement_plan:
				material_requirement_plan_map.setdefault(d.material_requirement_plan, []).append(d.material_requirement_plan_item)

		for mr, mr_item_rows in material_requirement_plan_map.items():
			if mr and mr_item_rows:
				mr_obj = frappe.get_doc("Material Requirement Plan", mr)

				if mr_obj.status in ["Stopped", "Cancelled"]:
					frappe.throw(
						_("{0} {1} is cancelled or stopped").format(_("Material Requirement Plan"), mr),
						frappe.InvalidStatusError,
					)
	...
	list_context.update(
		{
			...
			"show_search": True,
			"no_breadcrumbs": True,
			"title": _("Material Requirement Plan"),
		}
	)
...
def update_status(name, status):
	material_requirement_plan = frappe.get_doc("Material Requirement Plan", name)
	material_requirement_plan.check_permission("write")
	material_requirement_plan.update_status(status)
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Material Requirement Plan": {
				...
				"validation": {"docstatus": ["=", 1], "material_requirement_plan_type": ["=", "Purchase"]},
			},
			"Material Request Item Summary": {
				"doctype": "Purchase Order Item",
				"field_map": [
					["name", "material_request_item_summary"],
					["parent", "material_requirement_plan"],
					["uom", "stock_uom"],
					["uom", "uom"],
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			"Material Requirement Plan Item": {
				...
				"field_map": [
					["name", "material_requirement_plan_item"],
					["parent", "material_requirement_plan"],
...
def get_material_requirement_plans_based_on_supplier(doctype, txt, searchfield, start, page_len, filters):
	conditions = ""
	if txt:
	...
	material_requirement_plans = frappe.db.sql(
		"""select distinct mr.name, transaction_date,company
		from `tabMaterial Requirement Plan` mr, `tabMaterial Requirement Plan Item` mr_item
		where mr.name = mr_item.parent
			and mr_item.item_code in ({})
			and mr.material_requirement_plan_type = 'Purchase'
	...
	return material_requirement_plans


@frappe.whitelist()
@frappe.validate_and_sanitize_search_inputs
def get_default_supplier_query(doctype, txt, searchfield, start, page_len, filters):
	doc = frappe.get_doc("Material Requirement Plan", filters.get("doc"))
	item_list = []
	for d in doc.items:
...
def make_stock_entry(source_name, target_doc=None):
	def update_item(obj, target, source_parent):
		...
		if (
			source_parent.material_requirement_plan_type == "Material Transfer"
			or source_parent.material_requirement_plan_type == "Customer Provided"
		):
			target.t_warehouse = obj.warehouse
		else:
			target.s_warehouse = obj.warehouse

		if source_parent.material_requirement_plan_type == "Customer Provided":
			target.allow_zero_valuation_rate = 1

		if source_parent.material_requirement_plan_type == "Material Transfer":
			target.s_warehouse = obj.from_warehouse

	def set_missing_values(source, target):
		target.purpose = source.material_requirement_plan_type
		...
		if source.material_requirement_plan_type == "Customer Provided":
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Material Requirement Plan": {
				...
				"validation": {
					"docstatus": ["=", 1],
					"material_requirement_plan_type": [
						"in",
...
@frappe.whitelist()
def raise_work_orders(material_requirement_plan):
	mr = frappe.get_doc("Material Requirement Plan", material_requirement_plan)
	errors = []
	work_orders = []
				...
				wo_order.update(
					{
						...
						"material_requirement_plan": mr.name,
						"material_requirement_plan_item": d.name,
...
def create_pick_list(source_name, target_doc=None):
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Material Requirement Plan": {
				"doctype": "Pick List",
				"field_map": {"material_requirement_plan_type": "purpose"},
				"validation": {"docstatus": ["=", 1]},
			},
			"Material Requirement Plan Item": {
				"doctype": "Pick List Item",
				"field_map": {"name": "material_requirement_plan_item", "stock_qty": "stock_qty"},
			},
		},
		target_doc,
	)
