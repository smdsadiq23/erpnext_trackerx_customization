frappe.listview_settings["Material Requirement Plan"] = {
	add_fields: ["material_requirement_plan_type", "status", "per_ordered", "per_received", "transfer_status"],
	get_indicator: function (doc) {
		...
		if (doc.status == "Stopped") {
			...
			doc.material_requirement_plan_type == "Material Transfer"
		) {
			...
			} else if (doc.material_requirement_plan_type == "Purchase" && flt(doc.per_received, precision) == 100) {
				return [__("Received"), "green", "per_received,=,100"];
			} else if (doc.material_requirement_plan_type == "Purchase") {
				return [__("Ordered"), "green", "per_ordered,=,100"];
			} else if (doc.material_requirement_plan_type == "Material Transfer") {
				return [__("Transfered"), "green", "per_ordered,=,100"];
			} else if (doc.material_requirement_plan_type == "Material Issue") {
				return [__("Issued"), "green", "per_ordered,=,100"];
			} else if (doc.material_requirement_plan_type == "Customer Provided") {
